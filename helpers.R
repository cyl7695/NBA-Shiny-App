...
# Define a function for creating the plot
create_plot <- function(data, shot_type) {
  # Filter data for the specified shot type
  data <- data[data$shot_type == shot_type, ]
  
  # Calculate FG percentage
  fg_pct <- sum(data$shot_made_numeric) / nrow(data)
  
  # Create plot
  gg_court <- make_court()
  p <- gg_court + geom_point(data = data, alpha = 0.75, size = 2.5,
                             aes(loc_x, loc_y, color = shot_made_flag)) +
    scale_color_manual("", values = c(made = "blue", missed = "orange"))
  
  # Add FG percentage as a note on the plot
  p + annotate("text", x = Inf, y = Inf, label = paste0("FG%: ", round(fg_pct * 100, 1), "%"), 
               hjust = 3, vjust = 1, size = 6, col = "red")
}